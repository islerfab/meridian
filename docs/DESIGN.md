...
| Schema version | `meridian.v` | `X-MERIDIAN-V` | Marker format version (`2`; see marker version compatibility below). |
| Repair tries | `meridian.repair` | `X-MERIDIAN-REPAIR` | Bounded drift-repair attempt count against the current `Hash` (added v2, mer-t75). |
...
**Tamper-edit semantics (documented 2026-08-08 after Stage 2 hand-tamper testing, extended 2026-09-11 with bounded repair, mer-t75):** a consequence of marker-trust is that **manual edits to a shadow that preserve the marker are invisible** by hash comparison alone — the engine can't distinguish a hand-edit from provider-side normalization from the same fetch. Original response: **detect, don't repair** — each cycle hashes the already-fetched shadow content and sets `meridian_shadow_drift{rule,dest}` on mismatch (zero extra API calls, cannot loop; mer-hn8), with the edit persisting until the *source* event next changes or the operator runs `meridian wipe rule <id>`. The Stage 3 soak measurement (drift stable at zero for untouched events) showed provider normalization is hash-stable enough that unconditional repair-on-observed-mismatch was the actual risk to guard against — proven concretely by the mer-jge DST-fold bug, a deterministic provider-normalization mismatch that would have looped forever under naive repair. **Superseded 2026-09-11 (mer-t75): bounded repair.** A marker gains a `RepairTries` counter; on drift, the engine re-writes the already-desired content once per counter increment, capped at `maxRepairTries` (2) before falling back to permanent detect-only + a separate alertable `meridian_shadow_drift_unrepairable` metric. This self-heals genuine hand-edits while degrading safely into the original mer-hn8 behavior against a stable-but-wrong normalization — the bound is what makes repair safe, not a claim that observed-content comparison became trustworthy. Manual *deletes* are still always healed (missing → recreate) regardless. The `_synced by meridian_` description note (mer-06u) is the social layer of the same defense.

**Marker version compatibility (decided 2026-09-11, mer-dtd, after a live incident):** deploying the mer-t75 marker bump (v1→v2, additive: just the `RepairTries` field) against the running Stage 3 soak caused mass duplicate event creation, including on Fabio's real private Google calendar — `ParseMarker` hard-rejected any non-current version, so every pre-existing v1 shadow became invisible to the engine's ownership filter and `diff()` recreated everything from scratch, having no way to know it already existed. Rolled back; no lasting damage, but the failure mode generalizes to any future marker version bump against live state, including a real user's calendars post-beta. **Principle: backward-compatible reads, forward-only writes.** `ParseMarker` decodes every marker version it still knows how to interpret — an explicit per-version case, not a generic schema-evolution framework, since format changes should be rare — into the current in-memory `Marker`, defaulting new fields to their historically-correct value (a v1 marker's `RepairTries` is genuinely 0, not an unknown placeholder). `NewMarker`, the only write path, always stamps the current `MarkerVersion`; any shadow that legitimately gets touched (content-changed update, or drift-repair) upgrades to the current format as a side effect of normal reconciliation — no separate migration pass for additive changes. This also smooths over the harder case of a hash-*semantic* change (old markers' hashes no longer matching a new algorithm's output): it now just looks like drift and runs through the bounded repair path above instead of mass recreation — self-heals, or fails loudly into `meridian_shadow_drift_unrepairable` rather than silently duplicating content. **`meridian migrate` (not yet built) or a wipe (`meridian wipe`, mer-gih) is reserved for the one case this doesn't cover: a version bump whose old shape genuinely cannot decode into the new one at all** (a field removed or renamed, the ref encoding itself changed) — migrate only when backward compatibility isn't possible by principle, not as the default path for every version bump.
