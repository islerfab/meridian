...
**Tamper-edit semantics (documented 2026-08-08 after Stage 2 hand-tamper testing):** a consequence of marker-trust is that **manual edits to a shadow that preserve the marker are invisible** — the edit persists until the *source* event next changes (new desired hash → overwrite) or the operator runs `meridian wipe rule <id>` (next cycle recreates clean). Manual *deletes* are always healed (missing → recreate). This is the accepted trade-off, not a bug: comparing observed content instead would reintroduce the normalization-churn loop Decision 2 exists to avoid. Response chosen (with Fabio): **detect, don't repair** — each cycle hashes the already-fetched shadow content and sets `meridian_shadow_drift{rule,dest}` on mismatch (zero extra API calls, cannot loop; mer-hn8). The Stage 3 soak doubles as the measurement: drift stable at zero for untouched events ⇒ provider normalization is hash-stable and auto-repair could safely be added later; drift at rest ⇒ repair would loop, detect-only stands. The `_synced by meridian_` description note (mer-06u) is the social layer of the same defense.
