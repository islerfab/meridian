...
// newOAuthCmd performs the interactive OAuth bootstrap flow (mer-d5s):
...
func runOAuth(ctx context.Context, cmd *cli.Command) error {
	...
	if clientID == "" || clientSecret == "" {
		return errors.New("oauth: client ID and secret required (flags, env, or .env — see .env.example)")
	}
