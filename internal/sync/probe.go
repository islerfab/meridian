...
// write or delete anything. This is the readiness gate (DESIGN.md
// Decision 6): the run loop starts cycles only after a full probe pass, so
// an instance that never became ready has provably touched no calendar.
// Failures count into fetch_errors_total; the returned error joins one
// entry per failing calendar.
