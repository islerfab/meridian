...
// repair attempt heals it; without fresh tampering the next cycle is quiet.
...
// Drift detection + bounded repair: a hand-tampered shadow is repaired from
// the already-desired content, up to maxRepairTries times. If it's STILL
// drifted after the budget is exhausted, repair stops and the drift becomes
// visible via the unrepairable metric instead of retrying forever (the
// DST-fold class of bug proved unconditional retry unsafe). A real source
// change bypasses the budget entirely — it's diff()'s content-changed path,
// which always builds a fresh marker with RepairTries reset to 0.
func TestShadowDriftBoundedRepair(t *testing.T) {
	...
	// normalization) by tampering again before re-listing.
	tamper()
...
// Per-rule isolation — one rule's source failure must not affect another
// rule, and must cause no writes/GC for the failed rule.
...
// Duplicates: keep the hash-matching copy, delete the extras, one cycle
// restores the one-shadow-per-src invariant.
