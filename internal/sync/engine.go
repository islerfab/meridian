...
// maxRepairTries bounds drift-repair attempts per marker Hash. Not
// configurable: a constant is enough retry budget to absorb one cycle of
// provider read-after-write lag without risking an infinite update loop
// against a stable-but-wrong provider normalization.
...
type Config struct {
	...
	// as long as instance IDs differ.
	InstanceID string

	Rules    []Rule
	Adapters map[string]adapter.CalendarAdapter // by logical calendar ID

	// Sweepers enable account-wide sweep auto-GC: every cycle, each account
	// discovers ALL its calendars and deletes own-instance shadows whose
	// rule does not target that calendar in the current config. Keyed by
	...
	// Window bounds relative to cycle start: defaults 24h back, 90 days
	// ahead.
...
// shared per calendar across rules).
...
// sweep is the account-wide auto-GC: discover every calendar of every
// account, delete own-instance shadows (windowed) whose rule does not
// target that calendar in the current config. Guardrails: only own-instance
// markers are deleted, and shadows of an active (rule → calendar) pairing
// are never touched here — their rule's own reconciliation governs them,
// including when its cycle aborted. Past (out-of-window) shadows remain as
// history; unbounded cleanup is `meridian wipe`.
...
func (e *Engine) reconcileRule(ctx context.Context, rule Rule, window adapter.Window, src fetchResult[model.Event], shadows map[string]fetchResult[model.Shadow]) {
	...
	for _, dest := range rule.To {
		...
		for _, s := range destShadows.items {
			...
			// indistinguishable from here. Bounded repair: re-write the
			// already-desired content up to maxRepairTries times, stamping
			// the attempt into the new marker. A mismatch that survives
			// every attempt is either persistent hand-tamper or unstable
			// provider normalization (the DST-fold class of bug) — either
