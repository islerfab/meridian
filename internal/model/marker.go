...
// older one it still knows how to decode: backward-compatible reads,
// forward-only writes — a version bump alone must never make existing
// shadows invisible to the engine. NewMarker always stamps the current
// version; a version whose old shape can no longer be decoded into the
// current Marker at all is the one case that needs a real migration or a
// wipe, not silently handled here.
const MarkerVersion = 2

// Protocol selects which adapter's marker key scheme to use. A closed set
// of exactly the two protocols meridian speaks — a named type reads at call
// sites (ParseMarker(props, ProtocolGoogle)), where a bare bool would not.
type Protocol int
...
// Property keys for the marker on each protocol. Google:
// extendedProperties.private keys. CalDAV: X- properties on the shadow
// VEVENT.
const (
...
// attempts against this exact Hash — it resets to 0 whenever
...
//
// Backward-compatible reads, forward-only writes: every marker version this
// binary still knows how to decode is handled below via an explicit
// per-version case — deliberately not a generic schema-evolution framework,
// since format changes should be rare. src/rule/hash/instance/v are the
// version-independent core, present in every version so far; version-gated
// fields (repair, added in v2) are validated inside their version's case,
// with older versions defaulting them to their historically-correct value
