...
// source fetches, desired-set computation, and orphan GC. The zero Window
// means UNBOUNDED — no time filter at all — used only by wipe
// (ListShadows); ListEvents requires real bounds (server-side expansion
// needs them).
type Window struct {
	Start time.Time
	End   time.Time
...
// not. The engine's sweep auto-GC uses this to delete stale own-instance
// shadows account-wide.
...
// implement. No capability flags: protocol asymmetry stays inside each
// adapter, and all errors are normalized to the taxonomy in errors.go.
