...
// REPORT with <expand> (DESIGN.md Decision 3; gate-verified against
// sabre/dav 4.3.1 by hack/spike-expand). Shadow listing fetches the window
// and filters X-MERIDIAN-* markers client-side (Decision 5).
package caldav
...
type Config struct {
	...
	// principal lookups 404 (see DESIGN.md Decision 3 gate note).
	Username string
	Password string
...
func New(cfg Config, log *slog.Logger) (*Adapter, error) {
	...
	case "", "server":
	case "client":
		return nil, fmt.Errorf("calendar %s: client-side expansion is not implemented yet (expansion: client)", cfg.CalendarID)
	default:
		return nil, fmt.Errorf("calendar %s: unknown expansion mode %q (want server or client)", cfg.CalendarID, cfg.Expansion)
	}
...
// one PUT — last-writer-wins by design, Decision 5).
