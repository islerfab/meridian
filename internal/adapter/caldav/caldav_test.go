...
func TestNewRejectsClientExpansion(t *testing.T) {
	_, err := New(Config{
	...
	if err == nil {
		t.Fatal("expansion: client must be rejected until implemented (no silent fallback)")
	}
	if _, err := New(Config{
		Endpoint:     "https://example.com",
		CalendarPath: "/calendars/x/y/",
...
// Deletes are idempotent: already-gone (404/410) is success (Decision 5).
