...
// Regression: a shadow written by an older binary before X-MERIDIAN-REPAIR
// existed must still decode as owned, with RepairTries defaulting to 0 —
// not rejected as malformed. A MarkerVersion bump alone must never make
// pre-existing shadows invisible to the engine.
