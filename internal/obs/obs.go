// Package obs serves the observability HTTP surface (DESIGN.md Decision 6):
// /metrics (Prometheus), /healthz (liveness: process up), /readyz
// (readiness: config parsed, CEL compiled, auth probes passed). Readiness
// latches — once the startup auth probes pass it never flips back; later
// staleness is Prometheus's job (last_successful_cycle alert), not the
// kubelet's.
package obs
